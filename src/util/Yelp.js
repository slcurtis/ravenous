  ...
  getAccessToken() {
    if(accessToken) {
      ...
      return fetch(`https://cors-anywhere.herokuapp.com/api.yelp.com/oauth2/token?grant_type=client_credentials&client_id=${clientId}&client_secret=${secret}`, {
        method: 'POST'}).then(response => response.json()).then(jsonResponse => accessToken = jsonResponse.access_token);
    },

  search(term, location, sortBy) {
    //eslint-disable-next-line
    return Yelp.getAccessToken().then(() => fetch('https://cors-anywhere.herokuapp.com/api.yelp.com/v3/businesses/search?term=${term}location=${location}&sort_by=${sortBy}', {
